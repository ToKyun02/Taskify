...
import { Card, CardForm, cardSchema, CardsResponse, cardsResponseSchema, GetCardsParams, PutCardForm } from './types';
...
export const putCard = async (putCardForm: PutCardForm) => {
  const response = await axiosClientHelper.put<Card>(`/cards/${putCardForm.cardId}`, omit(putCardForm, ['cardId']));
  const result = cardSchema.safeParse(response.data);
  if (!result.success) throw new Error(RESPONSE_INVALID_MESSAGE);
  return result.data;
};
